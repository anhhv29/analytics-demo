...
import com.example.analytics1.databinding.ActivityNativeLargeOldBinding
...
class NativeLargeOldActivity : BaseActivity<ActivityNativeLargeOldBinding>() {
    override fun getActivityBinding() = ActivityNativeLargeOldBinding.inflate(layoutInflater)
        ...
        nativeManager?.loadNativeAd { nativeAd ->
            if (nativeAd != null) {
                nativeManager?.loadAdTemplate(
                    nativeAd,
                    R.layout.native_ad_template_large_old,
                    binding.layoutAds
                )
            }
        }
